...
#BSUB -J smore-6.7b-8e
#BSUB -o logs/smore-6.7b-8e.%J.o
#BSUB -e logs/smore-6.7b-8e.%J.e
#BSUB -q debug
...
if [[ $EP_SIZE -gt 1 ]]; then
    NAME="${NAME}-ep-${EP_SIZE}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}-test"
fi
...
data_options=" \
         ...
         --data-impl mmap"
export DATA_PATH=/gpfs/alpine2/stf218/world-shared/sajal/gptdata/gpttext_article_document
data_options="\
...
time jsrun --nrs 48 --tasks_per_rs 1 --cpu_per_rs 7 --gpu_per_rs 1 --rs_per_host 6 --latency_priority CPU-CPU --launch_distribution packed --bind packed:1 python pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} --num-workers 0
